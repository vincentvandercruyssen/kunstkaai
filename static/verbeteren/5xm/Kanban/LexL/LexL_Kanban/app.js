...
function kaartToevoegen() {
  ...
  // ✏️ Bewerken via dubbelklik
  tekstSpan.addEventListener("dblclick", () => {
    const nieuweTekst = prompt("Bewerk je kaart:", tekstSpan.textContent);
    if (nieuweTekst !== null && nieuweTekst.trim() !== "") {
      tekstSpan.textContent = nieuweTekst.trim();
    }
  });
